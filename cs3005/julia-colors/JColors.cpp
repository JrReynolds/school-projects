...
JColors::JColors(std::string infile, std::string outfile){
    ...
    std::vector<std::vector<int>> a = GenerateColors(10);
...
std::vector<std::vector<int>> & JColors::GenerateGradient(std::vector<int> & start, std::vector<int> & end, int step){
    ...
    int rDiff = (start[0] - end[0])/step;
    int gDiff = (start[1] - end[1])/step;
    int bDiff = (start[2] - end[2])/step;
    ...
    for(int i = 0; i < step; i++){
        int newR = start[0] - rDiff;
        int newG = start[1] - gDiff;
        int newB = start[2] - bDiff;
